...
python $OPENMNTPATH/translate.py -model $MODEL_DIR/best -src $PROJECT_DIR/data/${DATA_TYPE}/${AUG_TYPE}/${DATA_TYPE}.audio.${DATA_SECT}.src -verbose -output $OUTPUTS_DIR/${DATA_SECT}.out -batch_size 1 -beam_size 12
python $OPENMNTPATH/tools/wer.py -p $OUTPUTS_DIR/${DATA_SECT}.out -r $PROJECT_DIR/data/${DATA_TYPE}/${AUG_TYPE}/${DATA_TYPE}.audio.${DATA_SECT}.tgt > $OUTPUTS_DIR/${DATA_SECT}.out.wer  
