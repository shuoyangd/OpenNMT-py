...
stack_size: size of the stack LSTM
hid_dim: hidden dimension of the LSTMs
input_dim: 
action_emb_dim: 
word_emb_dim: 
max_step_length: don't need this at least for now, as we are not doing StackLSTM decoding
transSys: selection of transition systems listed above
exposure_eps: don't need 
num_lstm_layers: number of lstm layers for the StackLSTM
gpuid: 
...
class StackLSTMGenerator(nn.Module):
  ...
  def __init__(self, word_emb, actions, options, pre_vocab=None, postags=None):
    ...
    self.compose_tokens = nn.Sequential(
        nn.Linear(token_comp_in_features, options.stack_input_dim),
        nn.ReLU()
    )
    self.root_input = nn.Parameter(torch.rand(options.stack_input_dim))
  ...
  def forward(self, tokens, nmt_hiddens, pre_tokens=None, postags=None, actions=None):
    """
    ...
    :return: outputs: (output_seq_len, batch_size, len(actions)), or when actions = None, (output_seq_len, batch_size, max_step_length)
    """
    ...
    token_comp_output_rev = onmt.modules.utils.tensor.revert(token_comp_output, 0) # (seq_len, batch_size, input_dim), pad at beginning
