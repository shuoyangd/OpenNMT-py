...
class StackLSTMCell(nn.Module):
  ...
  def forward(self, input, op):
    ...
    :return: (hidden, cell): both are (batch_size, |A|, hidden_dim), where the cardinality of action space |A| is always 3.
    """
    ...
    possible_pos = torch.stack([self.pos - 1, self.pos, self.pos + 1], dim=0)  # (3, batch_size)
    hidden_ret = self.hidden_stack[possible_pos, batch_indexes.unsqueeze(0).expand(3, -1), :, :]
    cell_ret = self.hidden_stack[possible_pos, batch_indexes.unsqueeze(0).expand(3, -1), :, :]
  ...
  def head(self):
      return self.hidden_stack[self.pos, torch.arange(0, len(self.pos)).type(self.long_dtype), :, :] ,\
             self.cell_stack[self.pos, torch.arange(0, len(self.pos)).type(self.long_dtype), :, :]
