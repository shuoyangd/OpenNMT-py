...
def make_mask(lengths):
  ...
  :param lengths: LongTensor of sequence lengths (batch_size,)
  :return: ByteTensor of size (len(lengths), max(lengths))
  """
  
  batch_size = len(lengths)
  seq_len = torch.max(lengths).item()
  ret = torch.zeros(batch_size, seq_len + 1).byte()
  batch_indexes = torch.arange(0, batch_size).type_as(lengths)
  ret[batch_indexes, lengths] = 1
  ret = ret[:, :-1]
  ret = torch.cumsum(ret, dim=1)
  return 1 - ret
